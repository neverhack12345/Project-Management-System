...
description: >-
  ...
  (id, due, state, deps, facts, recur), research facts registry, Kanban lanes. Facts flow: create
  (add_project_fact) → assign (update_task_fact_refs or task factRefs) → update (update_project_fact).
  MCP: get_project_facts, add_project_fact, update_project_fact, update_task_fact_refs, plus list_projects,
  get_project, create_project, add_project_task, update_project_task, remove_project_task, delete_project,
  milestones, playbooks. npm: new:project, project:delete, project:facts, validate, ops:*. Use for
  project management, tasks, milestones, facts, portfolio dashboard, versionToken conflicts, or paths
  under projects/. Not for vault notes — use second-brain for vault/ and vault_* tools.
compatibility: >-
...
5. **Facts** — Registry in `research.md` (MCP `get_project_facts`, `add_project_fact`, `update_project_fact`). Assign with `update_task_fact_refs` (full merged `factRefs` array) or `add_project_task` / `update_project_task` `factRefs`. Unassigned = fact not in any `tasks[].factRefs` from `get_project`. CLI: `npm run project:facts -- list|create|update|assign` (no server). Dashboard: **Facts** tab + task detail fact links.
6. **Done gating** — `move_project_task_lane` or `update_project_task` to `done` is blocked while linked facts are unresolved; fix `research.md` / facts first.
7. **Delete project** — MCP `delete_project` or `npm run project:delete -- --slug <s> --confirm-slug <s>` (must match); removes `projects/<slug>/`.
8. **Ops / UI** — Reporting: [`references/cli-ops-reference.md`](references/cli-ops-reference.md). Dashboard: `/` when `npm run dev` is running.
...
- Fact: `get_project` → `add_project_fact` → `get_project` → `update_task_fact_refs` (merge ids) → `update_project_fact` when verifying.
